...
// Local Storage キー
var cred = "webmap-startup-template-js";
...
require([
	...
	Map,　Search,　Query,　FeatureLayer,　ServiceAreaParameters,　SpatialReference,　ServiceAreaTask,　SimpleFillSymbol,　SimpleLineSymbol,　SimpleMarkerSymbol,　FeatureSet,　GraphicsLayer,　Point,　Graphic,　Color,
	esriUtils,　urlUtils,
	on,　query,　dom,　domConstruct,　domClass,　domStyle,　array,　win,
	kernel, cookie, JSON, baseUnload,
	BootstrapMap,　dropdown,　modal,　tab,
	Chart,　Tooltip,　Highlight,　theme
	) {

		////**** ↓↓↓↓ 到達圏解析に必要な認証情報を Local Storage に保存（ログインを省略） ↓↓↓↓ ****////

        baseUnload.addOnUnload(storeCredentials);
        loadCredentials();

        // Local Storage に保存された認証情報を取得
		function loadCredentials() {
			var idJson, idObject;

			if ( supports_local_storage() ) {
			  // Local Storage から認証情報を取得
		...
		function runServiceAreaTask(mapPoint) {
			...
			serviceAreaTask.solve(params, function(solveResult){
				console.log("serviceAreaTask.solve: ", solveResult);
				// 解析結果
				...
				var serviceAreaSymbol = new SimpleFillSymbol(
					...
					new SimpleLineSymbol(
						...
						new Color([232,104,80,0]), 2
					),
					new Color([0,155,0,0.5])
				);
